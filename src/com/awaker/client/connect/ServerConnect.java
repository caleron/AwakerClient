...
public class ServerConnect {
    ...
    public ServerConnect(StatusChangedListener listener) {
        this.listener = listener;
        refreshPrefs();
    }

    public static void refreshPrefs() {
        String[] host = Prefs.getServer().split(":");
        hostIp = host[0];
    ...
    public void executeAction(Action action) {
        ...
        if (requestThread == null || !requestThread.isAlive()) {
            requestThread = new Thread(this::doRequests);
            requestThread.start();
        }
    }
        ...
        if (socket == null || !socket.isConnected() || socket.isClosed()) {
            try {
                ...
                if (lastNoConnectionAlertTime + timeout < currentTime) {
                    ...
                    listener.showError("Fehler: Verbindung fehlgeschlagen");
                }
            }
        }
    }
